...
def test_busca_tipo_ok(mock_conectar_banco, client, tipo):
    ...
    mock_cursor.execute.assert_called_once_with(
        "SELECT logradouro, tipo_logradouro, bairro, cidade, cep, tipo, valor, data_aquisicao FROM imoveis WHERE tipo = %s", (tipo,)
    )
...
@patch('api.conectar_banco')
def test_busca_tipo_vazio(mock_conectar_banco, client, tipo):
    mock_cursor = MagicMock()
    mock_conn = MagicMock()
    mock_conn.cursor.return_value = mock_cursor
